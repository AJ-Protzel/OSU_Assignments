 ...
 * - sends ciphertext to dec_server
 * - recives ciphertext from dec_server
...
/* checks length of ciphertext and key
 */
void checkLength(int sizeC, int sizeK){
  if(sizeK < sizeC){
...
/* checks valid chars in ciphertext and key
 */
void checkChars(char *ciphertext, char *key){
  ...
  if(fp == NULL){ // check exists
    ...
    while((ch = fgetc(fp)) != EOF && ch != '\n'){ // read through file, each char
      ...
      if(allowed == false){ // char is not allowed
        error("CLIENT: BAD: unallowed character in ciphertext\n", 1); 
      }
    }
  }
  fclose(fp);
...
/* concats ciphertext and key to send
 */
void makeMsg(char *buffer, char *ciphertext, int sizeC, char *key, int sizeK){
  FILE* fp;
  char line[MAX];

  strcpy(buffer, "D\n");

  fp = fopen(ciphertext, "r"); // get from ciphertext
  fgets(line, sizeC, fp);
  fclose(fp); 
...
int main(int argc, char *argv[]){
  // ./dec_client ciphertext key portnum
  checkArgs(argc); // pass argc
    int sizeC = getLength(argv[1]); // size of ciphertext
    int sizeK = getLength(argv[2]); // size of key
  checkLength(sizeC, sizeK); // pass ciphertext(argv[1]) and key(argv[2])
  checkChars(argv[1], argv[2]); // pass ciphertext(argv[1]) and key(argv[2])
  ...
  //         E  \n ciphertext \n   key   \n  eom \n  \0
  int size = 1 + 1 + sizeC + 1 + sizeK + 1 + 3 + 1 + 1;
  char *buffer = (char*) malloc((size)*sizeof(char));
  makeMsg(buffer, argv[1], sizeC, argv[2], sizeK);
  ...
  if(from[0] == 'B'){ // check if from dec
    error("DEC CLIENT: Message not from dec.\n", 2);
  }
  else{

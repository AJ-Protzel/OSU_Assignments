<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(empty($Ssn_err) && empty($Lname_err) && empty($Salary_err) && empty($Dno_err)&& empty($Address_err) && empty($Sex_err)){
            // Prepare an insert statement
            $sql = "INSERT INTO EMPLOYEE (Ssn, Fname, Lname, Address, Salary, Sex, Bdate, Dno) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            
            if($stmt = mysqli_prepare($link, $sql)){
                // Bind variables to the prepared statement as parameters
                mysqli_stmt_bind_param($stmt, "isssdssi", $param_Ssn, $param_Fname, $param_Lname, $param_Address, $param_Salary, $param_Sex, $param_Bdate, $param_Dno);

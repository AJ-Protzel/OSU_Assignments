...
# number_of_layers = 5
width_of_layers = 128  # only matters if number of layers > 1
...
def main():
  ...
  np.savetxt('results.csv', test_out, fmt='%s', delimiter=',')
...
# ################################################
  def backward(self, grad): 
    self.grad_weights = (np.transpose(self.input) @ grad) # = dL/dW
    self.grad_bias = np.sum(grad, axis=0)                 # = dL/db
    self.grad_input = (grad @ np.transpose(self.weights)) # = dL/dX

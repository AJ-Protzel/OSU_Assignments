...
dataset = np.zeros((150,2))

for i in range(1000):
    x = np.random.choice(len(dataset), replace=True)
    if(x < 0 or x > 150):
        print(bad)
    # else:
    #     print("good")
...
#   for i in range(50):
#     centroids, assignments, SSE = kMeansClustering(X, k, max_iters)
...
# # 
# # by selecting k random points from the dataset, 
# # but you are welcome to try alternatives.
# ##########################################################
# def initalizeCentroids(dataset, k):
#   # print('initalizeCentroids----------------------------------------------------------------')

#   centroids = np.zeros((k,dataset.shape[1]))
#   for i in range(k):
#     # centroids[i] = dataset[np.random.randint(0, high=dataset.shape[0])]
#     centroids[i] = np.random.choice(len(dataset), replace=False)
...
#   # print('computeAssignments----------------------------------------------------------------')

#   assignments = np.zeros((dataset.shape[0],1))
#   for i in range(len(assignments)):
#     dist = np.square(np.linalg.norm(dataset[i] - centroids, axis=1))
#     assignments[i] = np.argpartition(dist, range(1))[:1]
...
#     x = 0
#     y = 0
#     for ii in range(len(assignments)):
#       x += (i == assignments[ii])
#       y += (i == assignments[ii]) * dataset[ii]
#     counts[i] = x
#     centroids[i] = (1 / x) * y
...
#     sse += np.square(np.linalg.norm(dataset[i] - centroids[int(assignments[i])], axis=0))

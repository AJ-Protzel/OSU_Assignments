...
def initalizeCentroids(dataset, k):
  ...
  # centroids = np.array([])
  for i in range(k):
    centroids.append(dataset[np.random.choice(len(dataset), replace=True)])
    # np.append(centroids, dataset[np.random.choice(len(dataset), replace=True)])
...
def computeAssignments(dataset, centroids):
  ...
  #   #   d = np.linalg.norm(xi - centroids[cj])
  #   #   if(d < minimum):
  #   #     minimum = d
  #   #     idx = cj

    eucli_dist = np.linalg.norm(xi - centroids, axis=1)
    idx = np.argpartition(eucli_dist, k-1)[:k]
    assignments.append(idx[0])
...
def updateCentroids(dataset, centroids, assignments):
  ...
  for i in range(len(centroids)):
    ...
    for ii in range(len(assignments)):
      x = (i == assignments[ii]) ; y += (i == assignments[ii]) * dataset[ii]
    counts[i] = x
    centroids[i] = (1 / x) * y

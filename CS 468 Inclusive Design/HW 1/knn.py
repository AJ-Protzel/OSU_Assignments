...
def get_nearest_neighbors(example_set, query, k):
  dists = []
  for i in range(len(example_set)): # get distances
    dist = np.linalg.norm(example_set[i] - query)
    dists.append(tuple((dist,i))) # add tuple (distance, index)

  dists.sort() # sort by distance  
  x,dists = map(list, zip(*dists)) # filter out indexes
  dists = dists[:k]
...
def knn_classify_point(examples_X, examples_y, query, k):
  ...
  idx_of_nearest = get_nearest_neighbors(examples_X, query, k) # get nearest indexes
  
  labels = []
  for i in idx_of_nearest: # get labels
    labels.append(examples_y[i])

  if((np.mean(labels)) >= 0.5): # count most frequest label
    return 1
  return 0
...
def cross_validation(train_X, train_y, num_folds=4, k=1):
  ...
  data = np.split(train_X, num_folds) # split train_X
  true = np.split(train_y, num_folds) # split labels
  acc = []

  for skip in range(num_folds): # skip over one fold for testing
    test = []
    train = []
    pred = []
    test = data[skip]

    train = np.delete(data, skip, axis=0) # remove the skipped fold
    train = np.vstack(train)

    pred = predict(train, train_y, test, k) # get predicted labels
    acc.append(compute_accuracy(true[skip], pred))

  avg_val_acc = np.average(acc)
  var_val_acc = np.var(acc)

...
char *encrypt(char *buffer){
    ...
    for(int i = 0; i < strlen(lineP); ++i){ // loop plaintext number of times
      for(plainIndx = 0; lineP[i] != chars[plainIndx]; ++plainIndx){} // find num of plaintext char
      for(keyIndx = 0; lineK[i] != chars[keyIndx]; ++keyIndx){} // find num of key char
      int sum = (plainIndx + keyIndx) % 27; // encrypt it
      strncat(line, &chars[sum], 1); // add encrypted char into cipher line
    }
    // }
  ...
  while(1){ // accept a connection, blocking if one is not available until one connects
    ...
    switch (pid){
      ...
      case 0: // child
        ...
        if(buffer[0] == 'E'){
          strcpy(buffer, encrypt(buffer)); // encrypt plaintext
        }
        else{
          strcpy(buffer, "B\neom\n"); // bad message
        }

...
boot_alloc(uint32_t n)
{
	...
	if (!nextfree) {
		extern char end[];
		nextfree = ROUNDUP((char *) end+1, PGSIZE); // end+1 if mem error
	}
	...
	if(n == 0){
		...
		char* prev_nextfree = (char*)nextfree;
		nextfree = ROUNDUP(nextfree + n, PGSIZE); // n = 16KB

		if((PADDR(nextfree)) > npages * PGSIZE){ // if asked pages is more than memory
			panic("Out of memory");
		}
		else{
...
mem_init_mp(void)
{
    ...
    for(int i = 0; i < NCPU; ++i){
		uintptr_t start = KSTACKTOP - KSTKSIZE - i * (KSTKSIZE + KSTKGAP);
...
page_init(void)
{
	...
	// // >>>>>>>>>>>>>>>>>>>>>>
	// pages[0].pp_ref = 1; // first page must be in use
	...
	// // 	}		
	// // }
	// for (size_t i = 1; i < MPENTRY_PADDR/PGSIZE; i++) {
	// 	pages[i].pp_ref = 0;
	// 	pages[i].pp_link = page_free_list;
	// 	page_free_list = &pages[i];
	// }
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(pde&PTE_P){
		...
		if(pp_page_table == NULL || create == 0){
			return NULL;
		}
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	size_t nsize = ROUNDUP(size, PGSIZE);

    if(base + nsize >= MMIOLIM){
        panic("FAIL: no memory");
    }

    boot_map_region(kern_pgdir, base, nsize, pa, PTE_PCD | PTE_PWT | PTE_W);
    uintptr_t cur = base;
    base += nsize;

...
def main():
  ...
  w, bias_losses = trainLogistic(X_test_bias,y_train[:len(X_test_bias)])
  y_pred_test = X_test_bias@w >= 0 
  
  test_out = []
  for i in range(len(y_pred_test)):
    test_out.append([i, int(y_pred_test[i])])
  header = np.array([["id", "type"]])
  test_out = np.concatenate((header, test_out))
  np.savetxt('submission.csv', test_out, fmt='%s', delimiter=',')

######################################################################
# Q3.1 logistic 
######################################################################
# Given an (n x 1) input vector z, return a (n x 1) vector of the outputs of a logistic
...
def calculateNegativeLogLikelihood(X,y,w):
  # eq8: -sum(yi * logtheta(wTXi) + (1-yi) * log(1 - theta(wTXi)))
  nll = 0
  for i in range(len(X)):
    nll += y[i] * np.log(logistic(w.T @ X[i].T) + 0.0000001) + (1-y[i]) * np.log(1 - logistic(w.T @ X[i].T) + 0.0000001)
  return -nll
...
def trainLogistic(X,y, max_iters=max_iters, step_size=step_size):
    ...
    for i in range(max_iters):           
        # Todo: Compute the gradient over the dataset (Q10) and store in w_grad
        # XT(theta(Xw) - y)

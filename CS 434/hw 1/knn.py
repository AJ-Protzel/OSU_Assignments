...
def get_nearest_neighbors(example_set, query, k):
    dist = np.linalg.norm(example_set - query, axis=1) # get list of distances
    idx_of_nearest = np.argsort(dist)[:k] # get sorted list of idx's
...
def knn_classify_point(examples_X, examples_y, query, k):
    ...
    idx_of_nearest = get_nearest_neighbors(examples_X, query, k) # get nearest idx's

    labels = []
    for i in idx_of_nearest: # get labels
        labels.append(int(examples_y[i]))
    
    predicted_label = int((np.mean(labels)) >= 0.5) # count most frequest label
...
def cross_validation(train_x, train_y, num_folds=4, k=1):
    acc = []
    x = np.split(train_x, num_folds) # split train_x
    y = np.split(train_y, num_folds) # split train_y

    for skip in range(num_folds): # skip over one fold for testing
        train_X = np.vstack(np.delete(x, skip, axis=0))
        train_Y = np.vstack(np.delete(y, skip, axis=0))
        pred = predict(train_X, train_Y, x[skip], k) # get predicted labels
        acc = compute_accuracy(y[skip], pred)

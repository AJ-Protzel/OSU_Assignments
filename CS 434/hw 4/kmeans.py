...
def computeAssignments(dataset, centroids):
  assignments = []
  for i in dataset:
    eucli_dist = np.linalg.norm(centroids - i, axis=1)
    assignments.append(np.argmin(eucli_dist))
...
def updateCentroids(dataset, centroids, assignments):
  ...
  centroids = [np.mean(dataset[assignments == i], axis=0) for i in range(k)]
  centroids = np.vstack(centroids)
  counts = np.array([np.sum(assignments == i) for i in range(k)])
...
def calculateSSE(dataset, centroids, assignments):
  sse = np.sum(np.linalg.norm(dataset-centroids[assignments], axis =1) **2)
  return sse

...
step_size=0.00001 # 1, 0.1, 0.01, 0.001, [0.0001], 0.00001
max_iters=9000
...
def main():
  ...
  ####################################################
  # Q8 - Write the code to make your test submission here
  ####################################################
  ...
  np.savetxt("submission.csv", test_out, header="id,type", comments='', delimiter=",", fmt="%d")
...
def calculateNegativeLogLikelihood(X,y,w):
  l = logistic(X@w) + 0.0000000001
  nll = -y.T @ np.log(l) - (1-y).T @ np.log(1-l)
  return nll[0]
